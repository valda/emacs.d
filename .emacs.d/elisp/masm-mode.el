;;; Saved through ges-version 0.3.3dev at 2003-05-11 12:08
;;; ;;; From: myrkraverk@users.sourceforge.net (Jóhann Gunnar Óskarsson)
;;; ;;; Subject: Some changes to asm-mode
;;; ;;; Newsgroups: gnu.emacs.sources
;;; ;;; Date: 10 May 2003 02:40:45 +0000
;;; ;;; Organization: Internet a Islandi

;;; --=-=-=

;;; Hi,

;;; I've been fiddling a bit with asm mode, as I've found it a little 
;;; unusable.  The following file contains asm-mode.el (only with a new 
;;; name) with some changes I've made.

;;; I changed the name of the mode so even if I screwed something up, it 
;;; wouldn't be in the way of my work with assembly files.  (Inserting 
;;; comments didn't work at one time)

;;; These are the two most important changes I made:

;;; ;; in (defun masp-mode () ...
;;; ;; Recognise 'a' as string literal
;;; (modify-syntax-entry ?'  "\"" asm-mode-syntax-table)

;;; ;; in (defun asm-comment () ...
;;; ;; String?  Then insert comment char
;;; ((nth 3 (parse-partial-sexp (point-at-bol) (point)))
;;;    (insert asm-comment-char))

;;; I also changed the behaviour of asm-comment so that it wouln't move 
;;; point to the end of line after inserting the comment char into strings, 
;;; for details see a diff of masp-mode.el and asm-mode.el (I use the 21.3 
;;; version)
...
;;; Content-Disposition: attachment; filename=masp-mode.el
;;; Content-Description: Asm mode with changes
